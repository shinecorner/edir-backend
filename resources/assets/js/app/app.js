...
/* ==========================================================================
	...
	if(jQuery().fileuploader) {

        $('.image-uploader, .gallery-uploader').fileuploader({
            ...
            captions: {
                button: function (options) {
                    return 'Wählen Sie ' + (options.limit == 1 ? 'eine Datei' : 'Dateien');
                },
                feedback: function (options) {
                    return 'Wählen Sie ' + (options.limit == 1 ? 'Datei' : 'Dateien') + ' zum hochladen';
                },
                feedback2: function (options) {
                    return options.length + ' ' + (options.length > 1 ? ' Dateien wurden' : ' Datei wurde') + ' gewählt';
                },
                drop: 'Ziehen Sie Dateien auf dieses Feld um sie hochzuladen.',
                paste: '<div class="fileuploader-pending-loader"><div class="left-half" style="animation-duration: ${ms}s"></div><div class="spinner" style="animation-duration: ${ms}s"></div><div class="right-half" style="animation-duration: ${ms}s"></div></div> Datei wird eingefügt, hier klicken um abzubrechen.',
                removeConfirmation: 'Wollen Sie diese Datei tatsächlich entfernen?',
                errors: {
                    filesLimit: 'Es können nur ${limit} Dateien hochgeladen werden.',
                    filesType: 'Nur ${extensions} Dateien können hochgeladen werden.',
                    fileSize: '${name} ist zu groß! Bitte wählen Sie eine Datei mit maximal ${fileMaxSize}MB.',
                    filesSizeAll: 'Dateien, die Sie gewählt haben sind zu groß! Bitte wählen Sie Dateien mit maximal ${maxSize} MB.',
                    fileName: 'Eine Datei mit dem Name ${name} wurde bereits ausgewählt.',
                    folderUpload: 'Ordner können nicht hochgeladen werden.'
                }
            },
            thumbnails: {
                box: '<div class="fileuploader-items">' +
                '<ul class="fileuploader-items-list">' +
                '<li class="fileuploader-thumbnails-input"><div class="fileuploader-thumbnails-input-inner"><i class="fa fa-plus-circle"></i></div></li>' +
                '</ul>' +
                '</div>',
                ...
                onItemShow: function (item, listEl) {
                    ...
                    // @nmkr todo hide plus form when all items are filled..
